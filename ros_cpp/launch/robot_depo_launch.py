...
def generate_launch_description():
    ...
    nav2 = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            ...
            launch_arguments={'slam_params_file': '-r '+os.path.join(robot_path,'config/mapper_params_online_async.yaml')}.items(),
            
            #parameters=[robot_path+"/config/mapper_params_online_async.yaml"],
    )
    ...
    rviz = Node(
    	package='rviz2',
    	executable='rviz2',
	arguments=['-d',os.path.join(robot_path,'rviz','robot_depot.rviz')],
    )
    apriltags = Node(
